using System;
using System.Collections.Generic;
using System.Linq;
using System.Reactive.Linq;
using Genc;
using NUnit.Framework;

namespace Tests.GeneratorAPI {
    [TestFixture]
    internal class DecrementerTests {
        private static IEnumerable<int> GetExpectedDecrementationEnumerable(int start, int count) {
            // Closest way to copy the behaviour of decrementer.
            return Enumerable
                .Range((start * -1 + count) * -1 + 1, count)
                .Reverse();
        }

        [Test]
        public void Int_MinValue_Throws() {
            Assert.Throws<OverflowException>(() => Generator.Decrementer(int.MinValue)
                .Take(500)
                .ToEnumerable());
        }


        [Test]
        public void Start_Int_MaxValue_Does_Not_Throw() {
            Assert.DoesNotThrow(() => Generator.Decrementer(int.MaxValue)
                .Take(500).ToEnumerable());
        }

        [Test]
        public void Start_Minus_Twenty() {
            const int start = -20;
            const int count = 500;
            var result = Generator.Decrementer(start)
                .Take(count)
                .ToEnumerable();

            var expected = GetExpectedDecrementationEnumerable(start, count);

            Assert.AreEqual(expected, result);
        }

        [Test]
        public void Start_Twenty() {
            const int start = 20;
            const int count = 500;
            var result = Generator.Decrementer(start)
                .Take(count)
                .ToEnumerable();
            var expected = GetExpectedDecrementationEnumerable(start, count);
            Assert.AreEqual(expected, result);
        }


        [Test]
        public void To_Int_Min_Value() {
            var count = 500;
            var start = int.MinValue + count;
            var result = Generator.Decrementer(start)
                .Take(count)
                .ToEnumerable()
                .ToArray();
            var expected = GetExpectedDecrementationEnumerable(start, count);
            Assert.AreEqual(expected, result);
        }

        [Test]
        public void To_Less_Than_Int_Min_Value() {
            const int start = int.MinValue + 500;
            var count = Generator.Decrementer(start)
                .Take(502)
                .ToEnumerable()
                .Count();
            Assert.AreEqual(501, count);
        }
    }
}