...
namespace Genc {
    ...
    public static class Generator {
        ...
        public static IObservable<long> Randomizer(long min, long max, int? seed = null) {
            return max > min
                ? Observable.Generate(
                    new Tuple<Random, ulong>(CreateRandom(seed), (ulong) (max - min)),
                    tuple => true,
                    tuple => tuple,
                    tuple => {
                        ...
                        do {
                            var buf = new byte[8];
                            tuple.Item1.NextBytes(buf);
                            ulongRand = (ulong) BitConverter.ToInt64(buf, 0);
                        } while (ulongRand > ulong.MaxValue - (ulong.MaxValue % tuple.Item2 + 1) % tuple.Item2);

                        return (long) (ulongRand % tuple.Item2) + min;
                    }
                )
                : throw new ArgumentOutOfRangeException(nameof(max), @"max must be > min!");
